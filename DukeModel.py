...
# SWEEP VARIABLES
R_stu_range = np.array([1, 1.5, 2])
R_c = .5

screening_range = np.array([1, 2, 3]) / 7  # days per week

X_range = np.array([0, 1, 2])  # infections per week
Y_range = np.array([0, 3, 6])

VaccineDist = np.array([25, 50, 100]) / 7  # Daily vaccine distribution
...
def OnCampus(t, R_stu, screening, X, vaccineDist):
    ...
    if t % 7 == 0 and Susceptible_c[t] >= X:
        ...
        x = 0
    vaccinated = 0
    if vaccineDist and t >= vaccineDate:
        if Susceptible_o[t] - vaccineDist * vaccineEfficacy >= 0:
            vaccinated = vaccineDist * vaccineEfficacy
        else:
            Susceptible_o[t] = 0
    ...
    Susceptible_c[t + 1] = Susceptible_c[t] * (1 - Beta_stu * infectedFrac - Beta_c * infectedFrac_c) \
                           - Susceptible_c[t - 1] * screening * (1 - specificity) + mu * Iso_un_c[t] - x - vaccinated
...
def OffCampus(t, R_stu, screening, Y, vaccineDist):
    ...
    vaccinated = 0
    if vaccineDist and t >= vaccineDate:
        if Susceptible_c[t] - vaccineDist * vaccineEfficacy >= 0:
            vaccinated = vaccineDist * vaccineEfficacy
        else:
            Susceptible_c[t] = 0
            vaccinated = 0
    print(vaccinated, Susceptible_c[t])

    Susceptible_o[t + 1] = Susceptible_o[t] * (
            1 - Beta_stu * infectedFrac) - Susceptible_o[t - 1] * screening * (1 - specificity) + \
                           mu * Iso_un_o[t] - y - vaccinated
...
def Model(R_stu, screening, X, Y, vaccineDist, dir, sweptVar):
    ...
    print("Total Recovered {} On Campus: {}, Off Campus {}\n R:{}, Screening:{}, X:{}, Y:{},".format(
        Recovered_c[99] + Recovered_o[99], Recovered_c[99], Recovered_o[99], R_stu, screening, X, Y))
